...
** Starting the emacs server from here.
   If this is not done, the hooks for polypanel to show the current dektop do not show up. Probably because they are then not attached to the current emacs session, but to the systemctl emacs instance.
   #+begin_src elisp
     (server-start)
   #+end_src
